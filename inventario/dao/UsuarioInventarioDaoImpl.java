...
@Repository
@Transactional
public class UsuarioInventarioDaoImpl extends AbstractSession implements UsuarioInventarioDao{
	...
	//borrar usuario nombre
		public void deleteLogicoUsuarioByName(String nombre_completo) {
			BinUsuarioInventario usuario = findByName(nombre_completo);
			if (usuario != null) {			
				getSession().delete(usuario);	
			}
		}
	...
	public List<BinUsuarioInventario> findAllUsers(){
		return getSession().createQuery("select id_area_dgac, tipo_usuario, nombre_completo from BinUsuarioInventario where activo=1").list();
	}
